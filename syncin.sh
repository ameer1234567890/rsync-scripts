...
status="$?"
if [ "$status" != 0 ]; then
  rsync -rlpgohv --delete --progress --stats --partial --partial-dir=.rsync-partial --exclude-from /storage/emulated/0/Ameer/rsync-excludes.txt rsync://$host/usb1/Ameer/Phone/ /storage/emulated/0
  status="$?"
  if [ "$status" != 0 ]; then
