...
if [ "$status" != 0 ]; then
  ...
  status="$?"
  if [ "$status" != 0 ]; then
    rsync -ahv --delete --progress --partial --partial-dir=.rsync-partial --backup --backup-dir="/Ameer/Phone.old/backup_$(date +%Y-%m-%d_%H.%M.%S)" --exclude-from /storage/emulated/0/Ameer/rsync-excludes.txt /storage/emulated/0/ rsync://user@$host/usb1/Ameer/Phone
    status="$?"
    if [ "$status" != 0 ]; then
